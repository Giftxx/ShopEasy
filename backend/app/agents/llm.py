...
generated by a local Ollama model via its OpenAI-compatible API; every node
falls back to a template string if the call fails.
"""
...
def _make_ollama_client() -> tuple["OpenAI", str]:  # type: ignore[name-defined]
    """Return (client, model_name) for the configured Ollama endpoint."""
...
def call_llm(system_prompt: str, context: str, user_message: str) -> str | None:
    ...
    """
    try:
        client, model = _make_ollama_client()
...
def call_llm_classify(system_prompt: str, user_message: str) -> str | None:
    """
    Lightweight LLM call for classification tasks.
